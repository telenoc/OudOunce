...
class SaleOrder(models.Model):
    ...
    def _cart_update(self, product_id=None, line_id=None, add_qty=0, set_qty=0, **kwargs):
        ...
        if not order_line:
            ...
            values.update({
                'product_uom': self.env['multi.units'].browse(int(kwargs['uom'])).unit_id.id,
                'units_ids' : self.env['multi.units'].browse(int(kwargs['uom'])).id,
            })
        ...
        if quantity <= 0:
            ...
            if kwargs:
                ...
                uom = order_line.units_ids.id
                values.update({
                    'product_uom': order_line.units_ids.unit_id.id,
                    'units_ids' : order_line.units_ids.id
                })
            ...
            if self.pricelist_id.discount_policy == 'with_discount' and not self.env.context.get('fixed_price'):
                ...
                values['price_unit'] = self.env['account.tax']._fix_tax_included_price_company(
                    self._get_price(product, order_line, uom, pricelist),
    ...
    def _get_price(self, product=None, order_line=None, uom=0, pricelist=None):
        if int(uom) > 0:
            partner = self.env.user.partner_id
            pro = [(product, 1.0, partner)]
            muom = self.env['multi.units'].browse(int(uom))
            pricelist = self.env['product.pricelist'].browse(int(pricelist))
            price, rul_id = pricelist._compute_price_rule(pro, muom=muom)[product.id]
            return price
        else:
            return order_line._get_display_price(product)
